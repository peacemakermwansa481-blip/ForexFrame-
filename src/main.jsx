...
function BacktestPage({ user, onBack }) {
  ...
  if (mode === "session" && selected) return <BacktestSession backtest={selected} trades={trades} user={user} onBack={() => { setMode("landing"); setSelected(null); loadBacktests(); }} onRefresh={async (replayState = {}) => { if (Object.keys(replayState).length) { const status = replayState.replayStatus === "completed" ? "completed" : replayState.replayStatus === "running" || replayState.replayStatus === "paused" ? "in_progress" : "draft"; const { error: updateError } = await supabase.from("backtests").update({ status, replay_index: replayState.replayIndex, replay_balance: replayState.replayBalance, replay_position: replayState.replayPosition, replay_status: replayState.replayStatus }).eq("id", selected.id).eq("user_id", user.id); if (updateError) setError(updateError.message); setSelected((current) => ({ ...current, status, replay_index: replayState.replayIndex, replay_balance: replayState.replayBalance, replay_position: replayState.replayPosition, replay_status: replayState.replayStatus })); } await loadTrades(selected); await loadBacktests(); }} />;
