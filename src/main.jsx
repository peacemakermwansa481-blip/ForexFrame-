...
function BacktestPage({ user, onBack }) {
  ...
  async function persistReplayState(replayState = {}) {
    let persistenceError = null;
    if (Object.keys(replayState).length) {
      const status = replayState.replayStatus === "completed" ? "completed" : replayState.replayStatus === "running" || replayState.replayStatus === "paused" ? "in_progress" : "draft";
      const { error: updateError } = await supabase.from("backtests").update({ status, replay_index: replayState.replayIndex, replay_balance: replayState.replayBalance, replay_position: replayState.replayPosition, replay_status: replayState.replayStatus }).eq("id", selected.id).eq("user_id", user.id);
      persistenceError = updateError?.message || null;
      if (updateError) setError(updateError.message);
      console.info(persistenceError ? "[ForexFrame Replay] Supabase persistence failure" : "[ForexFrame Replay] Supabase persistence success", { backtestId: selected.id, replayState, error: persistenceError });
      setSelected((current) => ({ ...current, status, replay_index: replayState.replayIndex, replay_balance: replayState.replayBalance, replay_position: replayState.replayPosition, replay_status: replayState.replayStatus }));
    }
    await loadTrades(selected);
    await loadBacktests();
  ...
  if (mode === "session" && selected) return <BacktestSession backtest={selected} trades={trades} user={user} onBack={() => { setMode("landing"); setSelected(null); loadBacktests(); }} onRefresh={persistReplayState} />;
...
function ReplayCandleChart({ replay, trades = [], onSelectTrade }) {
  ...
  return <div className="replay-chart-wrap"><svg className="replay-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Revealed DEMO candlestick replay chart">{[0, .25, .5, .75, 1].map((step) => <g key={step}><line x1={pad.left} x2={width - pad.right} y1={y(max - range * step)} y2={y(max - range * step)} className="chart-grid-line" /><text x={pad.left - 7} y={y(max - range * step) + 4} textAnchor="end">{(max - range * step).toFixed(2)}</text></g>)}{candles.map((candle, index) => { const rising = candle.close >= candle.open; const bodyTop = y(Math.max(candle.open, candle.close)); const bodyHeight = Math.max(2, Math.abs(y(candle.open) - y(candle.close))); return <g key={candle.timestamp} className={index === candles.length - 1 ? "replay-current-candle" : ""}><line x1={x(index)} x2={x(index)} y1={y(candle.high)} y2={y(candle.low)} className={`candle-wick ${rising ? "candle-up" : "candle-down"}`} /><rect x={x(index) - candleWidth / 2} y={bodyTop} width={candleWidth} height={bodyHeight} className={`candle-body ${rising ? "candle-up" : "candle-down"}`} /></g>; })}{trades.flatMap((trade) => { const entryIndex = candles.findIndex((candle) => candle.timestamp === trade.entry_time); const exitIndex = candles.findIndex((candle) => candle.timestamp === trade.trade_date); return [{ trade, index: entryIndex, price: trade.entry, type: "entry" }, { trade, index: exitIndex, price: trade.exit_price, type: "exit" }].filter((marker) => marker.index >= 0 && marker.price !== null && marker.price !== undefined); }).map((marker) => <circle key={`${marker.trade.id}-${marker.type}`} cx={x(marker.index)} cy={y(Number(marker.price))} r="6" className={`trade-marker ${marker.type}`} onClick={() => onSelectTrade?.(marker.trade)}><title>{`${marker.type} ${marker.trade.direction} — ${Number(marker.price).toFixed(2)}`}</title></circle>)}{replay.position && <><line x1={pad.left} x2={width - pad.right} y1={y(replay.position.entry)} y2={y(replay.position.entry)} className="replay-entry-line" /><line x1={pad.left} x2={width - pad.right} y1={y(replay.position.stopLoss)} y2={y(replay.position.stopLoss)} className="replay-stop-line" /><line x1={pad.left} x2={width - pad.right} y1={y(replay.position.takeProfit)} y2={y(replay.position.takeProfit)} className="replay-target-line" /></>}<text x={pad.left} y={height - 12}>{new Date(candles[0].timestamp).toLocaleDateString()}</text><text x={width - pad.right} y={height - 12} textAnchor="end">{new Date(candles.at(-1).timestamp).toLocaleDateString()}</text></svg><div className="replay-chart-legend"><span><i className="legend-up" /> Up</span><span><i className="legend-down" /> Down</span><span><i className="legend-entry" /> Entry marker</span><span><i className="legend-target" /> Exit marker</span>{replay.position && <><span><i className="legend-entry" /> Open entry</span><span><i className="legend-stop" /> SL</span><span><i className="legend-target" /> TP</span></>}</div></div>;
}

function ReplayPanel({ backtest, user, completedTrades = [], onSelectTrade, onRefresh }) {
  const initialReplay = { status: "idle", index: -1, totalCandles: 0, visibleCandles: [], currentCandle: null, balance: Number(backtest.starting_balance), equity: Number(backtest.starting_balance), unrealizedPnL: 0, position: null, progress: 0, maxDrawdown: 0 };
  const [controller, setController] = useState(null);
  const [replay, setReplay] = useState(initialReplay);
  const [error, setError] = useState("");
  const [speed, setSpeed] = useState("normal");
  const [form, setForm] = useState({ direction: "Buy", stopLoss: "", takeProfit: "", riskPercent: String(backtest.risk_per_trade || 1), entryReason: "", exitReason: "", emotion: "", mistake: "", lesson: "" });
  const [diagnostics, setDiagnostics] = useState({ mounted: false, controllerCreated: false, loadingCandles: false, candleLoadCompleted: false, candlesLoaded: 0, lastReplayError: "", lastSupabasePersistenceError: "" });
  const timer = React.useRef(null);

  const recordReplayError = (replayError) => {
    const message = replayError instanceof Error ? replayError.message : String(replayError);
    console.error("[ForexFrame Replay] runtime error", replayError);
    setError(message);
    setDiagnostics((current) => ({ ...current, lastReplayError: message, loadingCandles: false }));
  };

  const publish = async (snapshot) => {
    const replayState = { replayIndex: snapshot.index, replayBalance: snapshot.balance, replayPosition: snapshot.position, replayStatus: snapshot.status };
    setReplay(snapshot);
    console.info("[ForexFrame Replay] replay state persistence", { backtestId: backtest.id, replayState });
    try {
      const result = await onRefresh(replayState);
      if (result?.error) throw new Error(result.error);
      console.info("[ForexFrame Replay] Supabase persistence success", { backtestId: backtest.id, replayState });
      setDiagnostics((current) => ({ ...current, lastSupabasePersistenceError: "" }));
    } catch (persistenceError) {
      const message = persistenceError instanceof Error ? persistenceError.message : String(persistenceError);
      console.error("[ForexFrame Replay] Supabase persistence failure", { backtestId: backtest.id, replayState, error: persistenceError });
      setDiagnostics((current) => ({ ...current, lastSupabasePersistenceError: message }));
    }
  };

  useEffect(() => {
    const config = { instrument: backtest.instrument, timeframe: backtest.timeframe, startDate: backtest.start_date, endDate: backtest.end_date, startingBalance: backtest.starting_balance, strategyName: backtest.strategy_name };
    console.info("[ForexFrame Replay] ReplayPanel mount", { backtestId: backtest.id, config });
    setDiagnostics((current) => ({ ...current, mounted: true, loadingCandles: true, candleLoadCompleted: false, candlesLoaded: 0, lastReplayError: "" }));
    let active = true;
    let nextController;
    try {
      nextController = new ReplayController({ provider: new MockHistoricalDataProvider(), config });
      console.info("[ForexFrame Replay] ReplayController creation success", { backtestId: backtest.id });
      setDiagnostics((current) => ({ ...current, controllerCreated: true }));
    } catch (controllerError) {
      console.error("[ForexFrame Replay] ReplayController creation failure", { backtestId: backtest.id, error: controllerError });
      recordReplayError(controllerError);
      return () => { active = false; if (timer.current) clearInterval(timer.current); };
    }
    nextController.load().then(() => {
      if (!active) return;
      const snapshot = nextController.restore({ index: backtest.replay_index ?? -1, balance: backtest.replay_balance, position: backtest.replay_position, status: backtest.replay_status || "idle" });
      console.info("[ForexFrame Replay] setController()", { backtestId: backtest.id, candles: nextController.candles.length });
      setDiagnostics((current) => ({ ...current, loadingCandles: false, candleLoadCompleted: true, candlesLoaded: nextController.candles.length }));
      setController(nextController);
      setReplay(snapshot);
    }).catch((loadError) => {
      if (!active) return;
      console.error("[ForexFrame Replay] ReplayController.load() failure", { backtestId: backtest.id, error: loadError });
      setDiagnostics((current) => ({ ...current, loadingCandles: false, candleLoadCompleted: true, candlesLoaded: nextController?.candles?.length || 0, lastReplayError: loadError.message || String(loadError) }));
      setError(loadError.message || String(loadError));
    });
    return () => { active = false; console.info("[ForexFrame Replay] ReplayPanel unmount", { backtestId: backtest.id }); if (timer.current) clearInterval(timer.current); };
  }, [backtest.id]);
  ...
  const advance = async () => { if (!controller) return; try { const result = controller.next(); await publish(result.snapshot); await saveTrade(result.trade); } catch (advanceError) { recordReplayError(advanceError); } };
  const start = () => { if (!controller) return; void publish(controller.start()); if (replay.index < 0) void advance(); };
  const pause = () => { if (timer.current) clearInterval(timer.current); if (controller) void publish(controller.pause()); };
  const play = () => { if (!controller) return; void publish(controller.start()); if (timer.current) clearInterval(timer.current); timer.current = setInterval(advance, speed === "slow" ? 1800 : speed === "fast" ? 350 : 900); };
  const reset = async () => { if (!controller || !window.confirm("Reset replay progress and remove this replay's simulated trades? The backtest configuration will remain.")) return; if (timer.current) clearInterval(timer.current); const { error: deleteError } = await supabase.from("backtest_trades").delete().eq("backtest_id", backtest.id).eq("user_id", user.id); if (deleteError) return recordReplayError(deleteError); void publish(controller.reset()); await onRefresh(); };
  const open = () => { try { const price = replay.currentPrice; void publish(controller.openPosition({ direction: form.direction, entry: price, stopLoss: Number(form.stopLoss), takeProfit: Number(form.takeProfit), riskPercent: Number(form.riskPercent) })); } catch (openError) { recordReplayError(openError); } };
  const close = async () => { try { const trade = controller.closePosition("Manual close"); void publish(controller.snapshot()); await saveTrade(trade); } catch (closeError) { recordReplayError(closeError); } };
  const candle = replay.currentCandle;
  return <section className="card replay-panel"><div className="replay-heading"><div><p className="eyebrow">DEMO DATA — NOT REAL MARKET DATA</p><h2>Candle replay</h2><p className="muted">Only revealed candles are rendered. Future candles remain unavailable.</p></div><span className="badge">{replay.status}</span></div><section className="replay-diagnostics" aria-live="polite"><div className="card-header"><div><p className="eyebrow">TEMPORARY RUNTIME TELEMETRY</p><h3>Replay Diagnostics</h3></div><span className="badge">{diagnostics.mounted ? "mounted" : "not mounted"}</span></div><div className="replay-diagnostics-grid"><div><span>ReplayPanel mounted</span><strong>{diagnostics.mounted ? "YES" : "NO"}</strong></div><div><span>Backtest ID</span><strong>{backtest.id || "Unavailable"}</strong></div><div><span>Instrument</span><strong>{backtest.instrument || "Unavailable"}</strong></div><div><span>Timeframe</span><strong>{backtest.timeframe || "Unavailable"}</strong></div><div><span>Start date</span><strong>{backtest.start_date || "Unavailable"}</strong></div><div><span>End date</span><strong>{backtest.end_date || "Unavailable"}</strong></div><div><span>Controller created</span><strong>{diagnostics.controllerCreated ? "YES" : "NO"}</strong></div><div><span>Loading candles</span><strong>{diagnostics.loadingCandles ? "YES" : "NO"}</strong></div><div><span>Candle load completed</span><strong>{diagnostics.candleLoadCompleted ? "YES" : "NO"}</strong></div><div><span>Number of candles loaded</span><strong>{diagnostics.candlesLoaded}</strong></div><div><span>Current replay index</span><strong>{replay.index}</strong></div><div><span>Replay status</span><strong>{replay.status}</strong></div><div><span>Current balance</span><strong>{Number(replay.balance).toFixed(2)}</strong></div><div><span>Position open</span><strong>{replay.position ? "YES" : "NO"}</strong></div><div className="full-width"><span>Last replay error</span><strong>{diagnostics.lastReplayError || "None"}</strong></div><div className="full-width"><span>Last Supabase persistence error</span><strong>{diagnostics.lastSupabasePersistenceError || "None"}</strong></div></div></section>{error && <p className="form-error">Replay data could not be loaded: {error}</p>}<ReplayCandleChart replay={replay} trades={completedTrades} onSelectTrade={onSelectTrade} /><div className="replay-progress"><div><strong>Replay progress</strong><span>{replay.index + 1} / {replay.totalCandles || "—"} candles · {replay.progress.toFixed(0)}%</span></div><div className="analytics-bar-track"><div className="analytics-bar" style={{ width: `${replay.progress}%` }} /></div></div><div className="replay-controls"><button className="primary-button" onClick={start} disabled={!controller || replay.index >= 0}>Start Replay</button><button className="secondary-button" onClick={() => { if (controller) void publish(controller.previous()); }} disabled={!controller || replay.index <= 0 || replay.position}>Previous Candle</button><button className="primary-button replay-next" onClick={advance} disabled={!controller || replay.index >= replay.totalCandles - 1}>NEXT CANDLE</button><button className="secondary-button" onClick={play} disabled={!controller || replay.status === "completed"}>{replay.status === "paused" ? "Resume" : "Play"}</button><button className="secondary-button" onClick={pause}>Pause</button><button className="secondary-button" onClick={reset}>Reset Replay</button></div><label className="replay-speed">Playback speed<select value={speed} onChange={(event) => setSpeed(event.target.value)}><option value="step">1 candle at a time</option><option value="slow">Slow</option><option value="normal">Normal</option><option value="fast">Fast</option></select></label><div className="replay-state-grid"><div><span>Current date/time</span><strong>{candle ? new Date(candle.timestamp).toLocaleString() : "Not started"}</strong></div><div><span>Instrument / timeframe</span><strong>{backtest.instrument} · {backtest.timeframe}</strong></div><div><span>OHLC</span><strong>{candle ? `${candle.open.toFixed(2)} / ${candle.high.toFixed(2)} / ${candle.low.toFixed(2)} / ${candle.close.toFixed(2)}` : "—"}</strong></div><div><span>Balance / equity</span><strong>{replay.balance.toFixed(2)} / {replay.equity.toFixed(2)}</strong></div><div><span>Current drawdown</span><strong>{replay.maxDrawdown.toFixed(2)}</strong></div><div><span>Open position</span><strong>{replay.position ? `${replay.position.direction} · ${replay.unrealizedPnL.toFixed(2)} P/L` : "None"}</strong></div></div><div className="replay-trade-box"><h3>{replay.position ? "Open simulated position" : "Open simulated trade"}</h3>{replay.position ? <div className="open-position-card"><p><strong>{replay.position.direction}</strong> · Entry {replay.position.entry.toFixed(2)} · Current {replay.currentPrice.toFixed(2)}</p><p>SL {replay.position.stopLoss.toFixed(2)} · TP {replay.position.takeProfit.toFixed(2)} · Size {replay.position.positionSize.toFixed(4)} · Risk {replay.position.riskPercent}%</p><p>Unrealized P/L <strong>{replay.unrealizedPnL.toFixed(2)}</strong> · R {replay.position.riskAmount ? (replay.unrealizedPnL / replay.position.riskAmount).toFixed(2) : "0.00"}</p><div className="replay-notes-grid"><label>Exit reason<input value={form.exitReason} onChange={(event) => setForm({ ...form, exitReason: event.target.value })} /></label><label>Emotion<input value={form.emotion} onChange={(event) => setForm({ ...form, emotion: event.target.value })} /></label><label>Mistake<input value={form.mistake} onChange={(event) => setForm({ ...form, mistake: event.target.value })} /></label><label>Lesson<input value={form.lesson} onChange={(event) => setForm({ ...form, lesson: event.target.value })} /></label></div><button className="primary-button" onClick={close}>Close Trade</button></div> : <div className="replay-form-grid"><label>Direction<select value={form.direction} onChange={(event) => setForm({ ...form, direction: event.target.value })}><option>Buy</option><option>Sell</option></select></label><label>Stop Loss<input type="number" step="any" value={form.stopLoss} onChange={(event) => setForm({ ...form, stopLoss: event.target.value })} /></label><label>Take Profit<input type="number" step="any" value={form.takeProfit} onChange={(event) => setForm({ ...form, takeProfit: event.target.value })} /></label><label>Risk %<input type="number" step="any" value={form.riskPercent} onChange={(event) => setForm({ ...form, riskPercent: event.target.value })} /></label><label className="full-width">Entry reason<textarea rows="2" value={form.entryReason} onChange={(event) => setForm({ ...form, entryReason: event.target.value })} /></label><button className="primary-button" onClick={open} disabled={!candle}>Open Trade at {replay.currentPrice.toFixed(2)}</button></div>}</div>{replay.status === "completed" && <div className="replay-complete"><h3>Backtest Complete</h3><p className="muted">The final candle is revealed. Final results are now available in the session summary after saved trades refresh.</p></div>}</section>;
}

function BacktestMiniChart({ points, valueKey, color, label, formatValue }) {
  if (!points || points.length < 2) return <div className="replay-chart-empty">No completed trade data yet.</div>;
  const width = 900; const height = 230; const pad = { left: 54, right: 18, top: 18, bottom: 30 }; const values = points.map((point) => Number(point[valueKey]) || 0); const min = Math.min(...values); const max = Math.max(...values); const range = Math.max(max - min, 0.01); const x = (index) => pad.left + (index / Math.max(points.length - 1, 1)) * (width - pad.left - pad.right); const y = (value) => pad.top + ((max - value) / range) * (height - pad.top - pad.bottom); const polyline = points.map((point, index) => `${x(index)},${y(Number(point[valueKey]) || 0)}`).join(" ");
  return <div className="backtest-mini-chart"><p className="chart-caption">{label}</p><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={label}>{[0, .5, 1].map((step) => <g key={step}><line x1={pad.left} x2={width - pad.right} y1={y(max - range * step)} y2={y(max - range * step)} className="chart-grid-line" /><text x={pad.left - 7} y={y(max - range * step) + 4} textAnchor="end">{formatValue(max - range * step)}</text></g>)}<polyline points={polyline} fill="none" stroke={color} strokeWidth="3" />{points.map((point, index) => <circle key={index} cx={x(index)} cy={y(Number(point[valueKey]) || 0)} r="4" fill={color}><title>{point.trade ? `${new Date(point.trade.trade_date).toLocaleString()} — ${formatValue(point[valueKey])}` : `Starting point — ${formatValue(point[valueKey])}`}</title></circle>)}<text x={pad.left} y={height - 8}>{points[0].trade ? new Date(points[0].trade.trade_date).toLocaleDateString() : "Start"}</text><text x={width - pad.right} y={height - 8} textAnchor="end">{points.at(-1).trade ? new Date(points.at(-1).trade.trade_date).toLocaleDateString() : "Start"}</text></svg></div>;
}

function BacktestTradeDetail({ trade, onClose }) {
  const display = (value) => value === null || value === undefined || value === "" ? "Unavailable" : value;
  const duration = trade.entry_time && trade.trade_date ? `${Math.max(0, (new Date(trade.trade_date) - new Date(trade.entry_time)) / 60000).toFixed(0)} minutes` : "Unavailable";
  return <div className="backtest-trade-detail"><div className="card-header"><div><p className="eyebrow">TRADE DETAIL</p><h3>{trade.direction} trade · {display(trade.outcome)}</h3></div><button className="secondary-button" type="button" onClick={onClose}>Close</button></div><div className="trade-detail-grid"><div><span>Date/time</span><strong>{display(new Date(trade.trade_date).toLocaleString())}</strong></div><div><span>Entry / exit</span><strong>{display(trade.entry)} / {display(trade.exit_price)}</strong></div><div><span>Stop / target</span><strong>{display(trade.stop_loss)} / {display(trade.take_profit)}</strong></div><div><span>P/L / R</span><strong>{display(trade.simulated_pnl)} / {display(trade.r_multiple)}</strong></div><div><span>Risk / size</span><strong>{display(trade.risk_percent)} / {display(trade.position_size)}</strong></div><div><span>Duration</span><strong>{duration}</strong></div><div><span>Strategy</span><strong>{display(trade.strategy)}</strong></div><div><span>Entry reason</span><strong>{display(trade.entry_reason)}</strong></div><div><span>Exit reason</span><strong>{display(trade.exit_reason)}</strong></div><div><span>Emotion</span><strong>{display(trade.emotion)}</strong></div><div><span>Mistake</span><strong>{display(trade.mistake)}</strong></div><div><span>Lesson</span><strong>{display(trade.lesson)}</strong></div></div></div>;
}

function CompletedBacktestResults({ trades, startingBalance, onSelectTrade, selectedTrade }) {
  const results = calculateBacktestResults(trades, startingBalance); const money = (value) => Number(value).toFixed(2); const currentDrawdown = results.drawdownCurve.at(-1)?.drawdown || 0;
  return <section className="completed-results"><div className="completed-banner"><p className="eyebrow">BACKTEST COMPLETE</p><h2>Completed replay results</h2><p className="muted">Final results are calculated only from completed replay trades.</p></div><div className="stats-grid completed-stats"><div className="card"><span>Starting balance</span><strong>{money(results.startingBalance)}</strong></div><div className="card"><span>Ending balance</span><strong>{money(results.endingBalance)}</strong></div><div className="card"><span>Net P/L</span><strong>{money(results.netPnL)}</strong></div><div className="card"><span>Return</span><strong>{results.returnPercent.toFixed(2)}%</strong></div><div className="card"><span>Max drawdown</span><strong>{money(results.maxDrawdown)}</strong></div><div className="card"><span>Max drawdown %</span><strong>{results.maxDrawdownPercent.toFixed(2)}%</strong></div><div className="card"><span>Total trades</span><strong>{results.totalTrades}</strong></div><div className="card"><span>Win rate</span><strong>{results.winRate.toFixed(1)}%</strong></div></div><section className="card result-section"><h2>Trade performance</h2><div className="result-metric-list"><span>Wins <strong>{results.wins}</strong></span><span>Losses <strong>{results.losses}</strong></span><span>Breakevens <strong>{results.breakevens}</strong></span><span>Average win <strong>{money(results.averageWin)}</strong></span><span>Average loss <strong>{money(results.averageLoss)}</strong></span><span>Largest win <strong>{money(results.largestWin)}</strong></span><span>Largest loss <strong>{money(results.largestLoss)}</strong></span><span>Average R <strong>{results.averageR.toFixed(2)}</strong></span><span>Profit factor <strong>{results.profitFactor === null ? "Unavailable" : results.profitFactor.toFixed(2)}</strong></span></div></section><section className="card result-section"><h2>Equity Curve</h2><BacktestMiniChart points={results.equityCurve} valueKey="balance" color="#60a5fa" label="Backtest balance over completed trades" formatValue={money} /></section><section className="card result-section"><h2>Drawdown</h2><div className="result-metric-list"><span>Maximum drawdown <strong>{money(results.maxDrawdown)}</strong></span><span>Maximum drawdown % <strong>{results.maxDrawdownPercent.toFixed(2)}%</strong></span><span>Final drawdown <strong>{money(currentDrawdown)}</strong></span></div><BacktestMiniChart points={results.drawdownCurve} valueKey="drawdown" color="#f87171" label="Drawdown progression" formatValue={money} /></section><section className="card result-section"><h2>Trade History</h2><div className="backtest-history-list">{trades.map((trade, index) => <button className="backtest-history-row" key={trade.id} type="button" onClick={() => onSelectTrade(trade)}><span>#{index + 1}</span><span>{new Date(trade.trade_date).toLocaleDateString()}</span><span>{trade.direction}</span><span>{money(trade.simulated_pnl)}</span><span>{trade.outcome}</span></button>)}</div>{selectedTrade && <BacktestTradeDetail trade={selectedTrade} onClose={() => onSelectTrade(null)} />}</section></section>;
}
